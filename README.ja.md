...
GitHub Copilot / Codex / Claude Code などのコーディングエージェントを、TailwindUSS を導入した Unity プロジェクトで使う場合は、`docs/AGENTS.md` をそのプロジェクトの `.github/AGENTS.md` としてコピーするか、各ツールの既存 instruction ファイルに内容を取り込んでください。

あわせて `docs/SKILL.md` も `.github/SKILL.md` としてコピーすると、TailwindUSS が対応している variant、utility family、token scale、Unity USS 由来の制約をエージェントへ知識として渡せます。
