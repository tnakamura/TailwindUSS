...
GitHub Copilot / Codex / Claude Code などのコーディングエージェントを、TailwindUSS を導入した Unity プロジェクトで使う場合は、`docs/agents/tailwinduss-ui-toolkit.md` をそのプロジェクトの `.github/agents/tailwinduss-ui-toolkit.md` としてコピーするか、各ツールの既存 instruction ファイルに内容を取り込んでください。

あわせて `docs/skills/tailwinduss-ui-toolkit/SKILL.md` も `.github/skills/tailwinduss-ui-toolkit/SKILL.md` としてコピーすると、TailwindUSS が対応している variant、utility family、token scale、Unity USS 由来の制約をエージェントへ知識として渡せます。

`docs/agents/tailwinduss-ui-toolkit.md` 側には、`tailwinduss.config.json` を先に読むこと、UXML では TailwindUSS utility class を優先すること、生成 USS を手編集しないこと、UI 変更後に `Tools/TailwindUSS/Generate` を実行すること、などをまとめています。
