...
- Unity 2022.3
- 現在のサポート対象は Unity 2022.3 のままです。Unity 6.3+ に切り上げた場合の追加候補 utility と移行計画は `docs/tailwind-uss-feature-matrix-ja.md` / `docs/tailwind-uss-implementation-plan-ja.md` を参照してください。
...
- 上記 2 文書には、Unity 6.3+ を前提にした filter 系 utility の再評価と詳細計画も含まれます。
...
- `font-*` weight: `thin`, `extralight`, `light`, `normal`, `medium` は `font-weight: normal`、`semibold`, `bold`, `extrabold`, `black` は `font-weight: bold` に集約
