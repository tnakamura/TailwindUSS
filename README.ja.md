...
この雛形には、`tailwinduss.config.json` を先に読むこと、UXML では TailwindUSS utility class を優先すること、生成 USS を手編集しないこと、UI 変更後に `Tools/TailwindUSS/Generate` を実行すること、などをまとめています。

具体例として、サンプルプロジェクトには `samples/TailwindUSS.Sample/AGENTS.md` も追加しています。
