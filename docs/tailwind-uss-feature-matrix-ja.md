...
| 余白 | `p-*`, `px-*`, `py-*`, `pt-*`, `pr-*`, `pb-*`, `pl-*`, `m-*`, `mx-*`, `my-*`, `mt-*`, `mr-*`, `mb-*`, `ml-*` | `padding-*`, `margin-*` |
...
| `gap-*`, `gap-x-*`, `gap-y-*` | ❌ | ❌ | Unity USS に `gap` family がない。child selector は使えても `:first-child` や sibling selector が無く、USS だけで厳密再現できない |
