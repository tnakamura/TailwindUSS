...
対象 Unity バージョンは、TailwindUSS パッケージ直下の Unity Package Manager 用 package manifest (`src/TailwindUSS/package.json`) に合わせて **Unity 2022.3** とする。
...
## 付録: Unity 6.3+ を前提にした filter 系 utility 実装計画

この節は、**サポート対象を Unity 6.3+ に引き上げる場合**に限って適用する追加計画である。現行 package manifest は `Unity 2022.3` のままなので、以下は future work として扱う。

### 目的

- Unity 6.3 の built-in `filter` を使って、Tailwind の filter 系 utility のうち **USS に素直に落とせる範囲**を追加する。
- 既存の `hover:` / `focus:` / `transition` と自然に組み合わせられる形で実装する。
- shader / custom filter asset 依存の強い機能は、MVP では対象外にする。
...
1. `src/TailwindUSS/package.json` と README が Unity 6.3+ を前提に更新されている。
2. filter 系 utility を複数指定した場合でも、**最終出力が 1 つの `filter` 宣言へ安定して合成**される。
3. `ValidationService` が unsupported filter family / invalid value / 重複 token を既存方針どおり診断できる。
4. `test/TailwindUSS.Editor.Tests/UtilityResolverTests.cs` に各 filter family の正常系・異常系テストがある。
5. `test/TailwindUSS.Editor.Tests/GenerationServiceTests.cs` に filter utility を複数含む end-to-end テストがある。
6. variant と transition の組み合わせを含む回帰テストが追加されている。

### Phase 6.3-0: ベースライン更新
...
1. サポート対象を Unity 6.3+ に更新する。
2. feature matrix の対象外リストから `filters` を分離し、built-in `filter` で直接表現できる範囲を明記する。
3. custom filter は stretch goal とし、初期スコープから外す。

#### 受け入れ条件

- 2022.3 前提の記述と 6.3 前提の記述が混在しない。
- README と matrix の説明が同じ対象範囲を指す。

### Phase 6.3-1: filter 合成基盤
...
1. `ResolvedUtility` か generation 用の内部モデルに、**単一 CSS property へ複数 token を合成できる表現**を追加する。
2. `blur-sm grayscale` のような複数 token を受けたとき、最終的に `filter: blur(...) grayscale(...)` のような 1 宣言へまとめる。
3. 合成順は固定化し、出力差分が毎回ぶれないようにする。
4. 同一 family の重複 (`blur-sm blur-lg` など) は既存方針どおり warning 対象にするか、後勝ちルールを明示してテストで固定する。

#### テスト追加

- resolver: filter token 単体解決
- generation: 複数 filter token を 1 つの `filter` 宣言へ合成
- validation: unsupported family / invalid value / duplicate

### Phase 6.3-2: 低コスト utility を追加する
...
1. Unity 6.3 built-in filters に直接対応する fixed / scale mapping を追加する。
2. Tailwind 側の scale を Unity が受け取れる単位に寄せる。
3. variant は既存実装をそのまま流用し、`hover:blur-sm` などの selector 生成に手を入れない。

#### テスト追加

- `UtilityResolverTests`: family ごとの正常系・異常系
- `GenerationServiceTests`: `hover:blur-sm grayscale invert-0 sepia`

### Phase 6.3-3: 中コスト utility を追加する
...
1. Tailwind 既定 scale のうち Unity 6.3 built-in filters へ素直に移せる値だけを初期実装に採用する。
2. `hue-rotate-*` は負値 variant を扱うなら family 設計を先に決め、`-hue-rotate-90` のような表現を受けるか別名に寄せるかを決める。
3. `contrast-*` / `hue-rotate-*` を既存 transition utility と組み合わせたとき、生成される USS が安定していることを確認する。

#### テスト追加

- `UtilityResolverTests`: scale mapping と invalid value
- `GenerationServiceTests`: `transition duration-150 hover:contrast-150 focus:hue-rotate-60`

### Phase 6.3-4: ドキュメントと回帰確認

#### 実施内容

1. README の utility subset に追加した filter family を追記する。
2. feature matrix で `Unity 6.3+ なら候補` としていた項目を実装済みに更新する。
3. unsupported として残す `drop-shadow-*` / `shadow-*` / `backdrop-*` / `brightness-*` / `saturate-*` の理由を文書に残す。
...
1. `blur-*`
2. `grayscale`, `invert`, `sepia`
3. `contrast-*`
4. `hue-rotate-*`
5. custom filter を伴う拡張の再検討

### 実装時の注意点

1. filter 系 utility は **token ごとに独立宣言を出す既存方式のままでは実装できない**。必ず合成レイヤーを先に入れる。
2. `opacity-*` は既存の `opacity` property utility と名前が衝突しやすいため、初期スコープに入れない。
3. built-in filter にない family を無理に追加せず、Tailwind 互換よりも **Unity 6.3 の標準機能で安定して出せる範囲**を優先する。
