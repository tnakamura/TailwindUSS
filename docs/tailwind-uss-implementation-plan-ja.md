...
2. `gap-*` は Unity USS の `gap` family 不在と structural selector 制約により一般 utility としては不採用にする。
...
- `gap-*` 系は unsupported warning を返すことを resolver / generation / validation で確認

実装済み:

- `flex-wrap*`, `items-stretch`, `justify-around`, `justify-evenly`, `self-*`, `basis-*`, `order-*`, `mt/mr/mb/ml`
...
2. `tracking-*` 用の新規 scale を追加する。`leading-*` は Unity USS に `line-height` がないため unsupported とする。
...
- `GenerationServiceTests` で `truncate tracking-wide` の end-to-end と `leading-*` unsupported warning を追加

実装済み:

- `text-left/right/center/justify`, `italic`, `whitespace-*`, `text-transform`, `tracking-*`
...
`truncate`, `inset-x-*`, `rounded-t-*` などは複数宣言を返す。`ResolvedUtility.Declarations` の順序は **テストで固定**し、emit 出力の安定性を維持する。
