...
対象 Unity バージョンは TailwindUSS パッケージ直下の UPM パッケージマニフェスト (`package.json`) に合わせて **Unity 2022.3** とする。
...
## Phase 1: 既存 parser のまま増やせる utility を追加する

variant や特殊 selector を伴わない、**単純な property マッピング**から着手する。

### 1-1. Layout / positioning
...
3. 現段階では escape syntax (`\:`) を解釈しなくてよい。TailwindUSS の class 記法では colon を variant separator 専用にする。
4. 未知 variant は `Unsupported utility token` ではなく、**variant unsupported** として warning を出せるよう issue kind を追加してよい。
...
## Phase 4: transform / transition / cursor
